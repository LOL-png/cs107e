...
typedef struct {
    ...
    unsigned long m_usec, f_usec; // elapsed usec within malloc/free
    int mcount, fcount; // op count malloc/free
...
static void run_workflow(int max_blocks, int max_size, int num_ops) {
    ...
    printf("Throughput:\n");
    printf("\t%5ld usec per malloc op \t(%d mallocs completed in %ld usec)\n",
        sim.m_usec/sim.mcount, sim.mcount, sim.m_usec);
    printf("\t%5ld usec per free op   \t(%d frees completed in %ld usec)\n",
        sim.f_usec/sim.fcount, sim.fcount, sim.f_usec);

    size_t sbrk_size = (char *)sim.seg_end - (char *)sim.seg_start;
    printf("Utilization:\n");
    printf("\tmalloc requests: total %ld payload bytes, peak in-use %ld payload bytes, sbrk extended %ld bytes\n",
        sim.aggregate, sim.peak, sbrk_size);
    printf("\t%4ld%% total payload/sbrk\t(over 100%% indicates recycling)\n", sim.aggregate*100/sbrk_size);
    printf("\t%4ld%% peak payload/sbrk\t(packing density, up to theoretical max of 100%%) \n", sim.peak*100/sbrk_size);
}
...
static void checked_malloc(size_t requested_size, sim_t *sim) {
    ...
    sim->m_usec += elapsed/TICKS_PER_USEC;
    sim->mcount++;
...
static void checked_free(int index, sim_t *sim) {
    ...
    free(p);
    unsigned long elapsed = timer_get_ticks() - start;
    sim->f_usec += elapsed/TICKS_PER_USEC;
    sim->fcount++;
